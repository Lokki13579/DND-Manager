...
class Character:
    def __init__(self, name="Выбирается", stats=None):
        ...
        try:
            ...
            if isinstance(statuses_data, dict):
                ...
                status_keys = ["Poisoned", "Blinded", "Frightened", "Grappled", "Incapacitated",
                             "Invisible", "Paralyzed", "Petrified", "Exhaustion", "Deafened",
                             "Stunned", "Unconscious", "Charmed", "Restrained", "Prone"]
            self.status = dict(zip(status_keys, [False for _ in range(len(status_keys))]))
        except:
            # Резервные статусы
            default_statuses = ["Poisoned", "Blinded", "Frightened", "Grappled", "Incapacitated"]
            self.status = dict(zip(default_statuses, [False for _ in range(len(default_statuses))]))
    ...
    def otherStatsReset(self):
        otherSt = {}
        try:
            class_data = jsonLoad("JSONS/dnd_classes.json").get(self.Stats.get("class", ""), {})
            ...
            for i in level_data:
                ...
                if i == "DifferentSpellCells":
                    ...
                    for spCell in level_data[i]:
                        cell_level = spCell[-1] if isinstance(spCell, str) and spCell else "1"
                        otherSt["MaxspellCells"][cell_level] = str(level_data[i].get(spCell, 0))
                    continue
                if i == "Ячейки заклинаний" or i == "Уровень ячеек":
                    otherSt["MaxspellCells"] = {}
                    cell_level = level_data.get("Уровень ячеек", "1")
                    otherSt["MaxspellCells"][cell_level] = level_data.get("Ячейки заклинаний", 0)
                    continue
            ...
            if "formula" in class_data:
                ...
                otherSt["Известные заклинания"] = max(1, math.floor((int(dice_stat) - 10) / 2) + math.floor(int(other_stat) / int(formula[2])))
    ...
    def spellCellsCh(self, key, value):
        ...
        try:
            ...
            if max_cells:
                ...
                max_cell_value = int(self.Stats.get("otherStats", {}).get("Ячейки заклинаний", 0))
