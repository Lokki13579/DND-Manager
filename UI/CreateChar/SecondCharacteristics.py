...
class SecondCharacteristics(QWidget):
    ...
    def setupUi(self):
        ...
        self.raceParamsTreeGroup = QTreeWidgetItem(self.raceParamsTree,[self.raceChoose.currentText()])
        self.raceData = {}
    ...
    def skillAdder(self,obj : QTreeWidgetItem,path: str|dict,_dict):
        ...
        self.skills = self.recursiveAddTreeItems(obj,self.character.Stats.get("skills"))
       
    def raceDataAdder(self,obj: QTreeWidgetItem,path,_dict):
        if type(path) == type(""):
            _info = jsonLoad(path)
        else: _info = path
        self.raceData = self.recursiveAddTreeItems(obj, _info.get(self.character.Stats.get("race")))
