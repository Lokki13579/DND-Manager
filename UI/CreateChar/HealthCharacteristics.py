...
class HealthCharacteristics(QWidget):
    ...
    2. Взять среднее значение +1 ({self.mid}) и прибавить к нему тот же модификатор""")
            ...
            self.CurrentVal.setText("Текущее кол-во хитов - "+str(self.character.getMaxHp()))
            ...
            self.MidButton.setText(f"Выбрать среднее значение ({self.mid + int(self.stat.modif)})")
        except:pass
    def setupUi(self):
    ...
    def randomHp(self):
        print(self.levels)
        if self.levels >= 1:
            randomizedHp = max([1,randint(1,int(self.dice[1:]))+int(self.stat.modif)])
